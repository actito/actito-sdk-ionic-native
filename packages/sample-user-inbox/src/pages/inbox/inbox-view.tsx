...
import { ActitoUserInbox } from 'capacitor-actito-user-inbox';
import type { ActitoUserInboxItem } from 'capacitor-actito-user-inbox';
...
export const InboxView: FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<ActitoUserInboxItem | null>(null);
  ...
  const refreshInbox = useCallback(async () => {
    try {
      ...
      const userInboxResponse = await ActitoUserInbox.parseResponseFromString(requestResponseStr);
  ...
  useEffect(
    function setupListeners() {
      const listeners = [
        ActitoPush.onNotificationOpened(async () => {
          await refreshInbox();
        }),

        ActitoPush.onNotificationInfoReceived(async () => {
          await refreshInbox();
        }),
      ];
  ...
  function onTouchStart(item: ActitoUserInboxItem) {
  ...
  async function onTouchEnd(item: ActitoUserInboxItem) {
    clearTimeout(pressTimer);
