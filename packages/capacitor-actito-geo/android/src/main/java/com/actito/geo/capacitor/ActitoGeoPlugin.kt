package com.actito.geo.capacitor
...
public class ActitoGeoPlugin : Plugin(), ActitoGeo.Listener {
    ...
    public fun hasLocationServicesEnabled(call: PluginCall) {
        call.resolve(
            JSObject().apply {
                put("result", Actito.geo().hasLocationServicesEnabled)
            }
        )
    }
    ...
    public fun hasBluetoothEnabled(call: PluginCall) {
        call.resolve(
            JSObject().apply {
                put("result", Actito.geo().hasBluetoothEnabled)
            }
        )
    }
    ...
    public fun getMonitoredRegions(call: PluginCall) {
        try {
            call.resolve(
                JSObject().apply {
                    put("result", JSArray(Actito.geo().monitoredRegions.map { it.toJson() }))
                }
            )
        } catch (e: Exception) {
    ...
    public fun getEnteredRegions(call: PluginCall) {
        try {
            call.resolve(
                JSObject().apply {
                    put("result", JSArray(Actito.geo().enteredRegions.map { it.toJson() }))
                }
            )
        } catch (e: Exception) {
            call.reject(e.localizedMessage)
